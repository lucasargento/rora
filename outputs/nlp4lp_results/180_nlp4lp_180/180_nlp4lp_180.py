...
def solve_linear_program():
    ...
    # Variables:
    # x_A: servings of supplement A (continuous, non-negative)
    # x_B: servings of supplement B (continuous, non-negative)
    ...
    # Objective: minimize total cost = cost_A * x_A + cost_B * x_B
    objective = solver.Objective()
    objective.SetCoefficient(x_A, cost_A)
    objective.SetCoefficient(x_B, cost_B)
    objective.SetMinimization()
    
    # Constraint 1: Calcium constraint: 30*x_A + 60*x_B >= 400
    constraint_calcium = solver.Constraint(min_Calcium, solver.infinity())
    constraint_calcium.SetCoefficient(x_A, calcium_A)
    constraint_calcium.SetCoefficient(x_B, calcium_B)
    
    # Constraint 2: Magnesium constraint: 50*x_A + 10*x_B >= 50
    constraint_magnesium = solver.Constraint(min_Magnesium, solver.infinity())
    constraint_magnesium.SetCoefficient(x_A, magnesium_A)
    constraint_magnesium.SetCoefficient(x_B, magnesium_B)
    ...
    # Prepare solution in the expected output format.
    solution = {
        "variables": {
            "Servings": {
                "0": x_A.solution_value(),
                "1": x_B.solution_value()
            }
        },
...
def main():
    # Only one formulation is provided, so we run a single model.
    solution_lp, error_lp = solve_linear_program()
    
    print("Results for the Linear Programming formulation:")
    if error_lp:
        print("Error:", error_lp)
    else:
        print("Optimal Servings of Supplement A (index '0'): {:.4f}".format(solution_lp["variables"]["Servings"]["0"]))
        print("Optimal Servings of Supplement B (index '1'): {:.4f}".format(solution_lp["variables"]["Servings"]["1"]))
        print("Minimized Total Cost (objective): {:.4f}".format(solution_lp["objective"]))
