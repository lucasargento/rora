...
def main():
    ...
    # Data: demand for each day of the week (days 1 to 7).
    demand = [0, 5, 9, 15, 4, 7, 3]
    days = range(1, 8)  # Days 1 to 7
    work_days = range(1, 8)  # x in {1,..,7}
    start_days = range(1, 8)  # s in {1,..,7}

    # Decision variables: y[x, s] is the number of nurses with pattern (x, s).
    y = {}
    for x in work_days:
        for s in start_days:
            y[(x, s)] = model.NewIntVar(0, 1000, f'y_{x}_{s}')

    # Constraint: For each day d ensure that enough nurses cover the demand.
    # A nurse with pattern (x,s) covers day d if ((d - s) mod 7) is in {0, 1, ..., x-1}.
    for d in days:
        coverage = []
        for x in work_days:
            for s in start_days:
                if ((d - s) % 7) < x:
                    coverage.append(y[(x, s)])
        model.Add(sum(coverage) >= demand[d - 1])

    # Objective: minimize the total number of nurses hired.
    model.Minimize(sum(y[(x, s)] for x in work_days for s in start_days))
    ...
    if status in (cp_model.FEASIBLE, cp_model.OPTIMAL):
        print('Optimal objective:', solver.ObjectiveValue())
        for x in work_days:
            for s in start_days:
                if solver.Value(y[(x,s)]) > 0:
                    print(f'Pattern (x={x}, s={s}): {solver.Value(y[(x,s)])}')
    else:
